...
class KGDataLoader:
    ...
    def __init__(self,
        ...
        self.logger.debug(f"Loaded {len(self.entity2name)} entity names")

        # 加载关系到ID映射
        self.relation2id = self._load_relation2id(relation2id_path)
        self.logger.debug(f"Loaded {len(self.relation2id)} relations")
        ...
        self.entity2description = self._load_entity2description(
            entity2description_path)
        self.logger.debug(
            f"Loaded {len(self.entity2description)} entity descriptions")

        # 加载知识图谱三元组
        self.kg_triplets = self._load_kg_triplets(kg_path)
        self.logger.debug(f"Loaded {len(self.kg_triplets)} KG triplets")

        # 构建邻接关系索引
        self._build_adjacency_index()
        self.logger.debug("Built adjacency index")
    ...
    def _load_entity2id(self, file_path: str) -> Dict[str, str]:
        """加载实体到ID映射"""
        entity2id = {}
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
    ...
    def _load_kg_triplets(self, file_path: str) -> Set[Tuple[str, str, str]]:
        ...
        try:
            ...
            self.logger.error(
                f"Error loading KG triplets from {file_path}: {e}")
            raise
        return kg_triplets
